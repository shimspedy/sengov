function xml_to_json_tables(url, tableid, tablediv) {
    ...
    fetch(apiUrl)
      ...
      .then((xmlString) => {
        ...
        let tabs = `
    <div class="tabs">
    <label class="tab active" id="${class_annual}">Annual</label>
    <label class="tab" id="${hourly_annual}">Hourly</label>
    <label class="tab" id="${overtme_annual}">Overtime</label>
      </div>`;
        let th_count =
          items[0].getElementsByTagName("Steps")[0].childElementCount;
        let th_html = "<tr><th>Leo Grade</th>";
  ...
  $(document).ready(function () {
    $("path, circle").click(function (e) {
      ...
      if (id != "AK" && id != "HI") {
        xml_to_json_tables(
          "https://www.opm.gov/policy-data-oversight/pay-leave/salaries-wages/salary-tables/xml/2023/RUS.xml",
          "table_rus",
          "table_rus"
        );
      }
